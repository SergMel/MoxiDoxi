...
fun main(args: Array<String>) {
    ...
    for (t in 1..readInt()) {
        ...
        val rows = List<HashMap<Int, Int>>(n){HashMap<Int, Int>(n)}
        val cols = List<HashMap<Int, Int>>(n){HashMap<Int, Int>(n)}
        var res = 0
        for (i in 0..n-1) {
            val lst = readInts()
            res+=lst[i]
            for (j in 0..n-1) {
                rows[i][lst[j]] = rows[i].getOrDefault(lst[j], 0) + 1
                cols[j][lst[j]] = cols[j].getOrDefault(lst[j], 0) + 1
            }
        }
        
        writeGoogleAnswer(t, "$res ${rows.asSequence().count({it.count() < n})} ${cols.asSequence().count({it.count() < n})}"  )
