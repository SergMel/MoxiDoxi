...
fun main(args: Array<String>) {
    ...
    for (t in 1..readInt()) {
        val n = readInt()
        val C = BooleanArray(24 * 60) { false }
        val J = BooleanArray(24 * 60) { false }
        val res = CharArray(n) { 'C' }
        var f = false
        val acts = mutableListOf<Triple<Int, Int, Int>>()
        
        for (a in 0..n-1) {
            val (s, e) = readInts()
            acts.add(Triple(s, e, a))
            
        }
        acts.sortBy { it.first }
        for (a in 0..n-1) {
            val (s,e) = acts[a]
            if(f){
                break
        ...
        if(f){
            ...
            val res2 = CharArray(n) { 'C' }

            for (el in 0 until acts.size) {
                res2[acts[el].third] = res[el]    
            }
            writeGoogleAnswer(t,res2.joinToString(separator="") )
        }
