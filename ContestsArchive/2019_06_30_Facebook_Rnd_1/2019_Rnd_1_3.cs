    ...
    static void Main(string[] args)
    {
        ...
        using (var stream = new FileStream("input.txt", FileMode.Open))
        {
            using (StreamReader reader = new StreamReader(stream))
            {
                ...
                {
                    var nh = reader.readInts();
                    var n = nh[0];
                    var h = nh[1];

                    Dictionary<int, List<Ladder>> dic = new Dictionary<int, List<Ladder>>();
                    Dictionary<int, List<Ladder>> sts = new Dictionary<int, List<Ladder>>();
                    Dictionary<int, List<Ladder>> ends = new Dictionary<int, List<Ladder>>();

                    for (int i = 0; i < n; i++)
                    {
                        var arr = reader.readInts();
                        ...
                        dic.GetOrAddNew(ladder.x).Add(ladder);
                        sts.GetOrAddNew(ladder.vs).Add(ladder);
                        ends.GetOrAddNew(ladder.ve).Add(ladder);
