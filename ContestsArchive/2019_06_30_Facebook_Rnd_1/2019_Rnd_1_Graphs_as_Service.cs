    ...
    static void Main(string[] args)
    {
        ...
        using (var stream = new FileStream("input.txt", FileMode.Open))
        {
            using (StreamReader reader = new StreamReader(stream))
            {
                ...
                for (int i = 0; i < t; i++)
                {
                    var tmp = reader.readInts();
                    var n = tmp[0];
                    var m = tmp[1];
                    var edges = reader.readWEdges(m);
                    edges = edges.OrderBy(el => el.w).ToList();
                    Graph graph = new Graph(n);
                    bool f = true;
                    foreach (var key in edges)
                    {
                        var dist = graph.Dijkstra(key.s, key.e);
                        if (dist != Graph.Invalid && dist < key.w)
                        {
                            f = false;
                            break;
                        }
                    ...
                    if (!f)
                    {
                        sb.writeCase(i + 1, "Impossible");
                    }
                    else
                    {
                        sb.writeCase(i + 1, graph.edges.Count.ToString());
                        sb.outputEdges(graph.edges);
                    }
