...
class C2019_Yandex_H
{
    ...
    static int binarySearch(BitSum bs, long weight)
    {
        var l = 0;
        var lval = bs.GetSum(l);
        if (lval > weight)
        {
            return -1;
        }
        var r = bs.Length - 1;
        var rval = bs.GetSum(r);
        if (rval <= weight)
        {
            return r;
        }
    ...
    static void Main(string[] args)
    {
        ...
        var n = arr[0];
        var q = arr[1];
        BitSum bs = new BitSum(n);
        for (int i = 0; i < q; i++)
        {
            var strs = readStrings();
            if (strs[0] == "+")
            {
                var shard = int.Parse(strs[1]);
                var weight = int.Parse(strs[2]);
                bs.Add(shard, weight);

            }
            else
            {
                var weight = long.Parse(strs[1]);
                var index = binarySearch(bs, weight);
